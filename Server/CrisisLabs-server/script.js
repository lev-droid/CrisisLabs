...
server.get('/edit', (req,res)=>{
	...
	// theres a native js implementation for checking if a specific value is in an array but im pretty sure it literally just does this
	for (int i = 0; i < tokens.length; i++)
	{
		if (tokens[i] == token)
		{
			bool = true; // we only want to set it to true if the token is in the array (rather than also setting it to false if the token isn't) so we have to set the bool in an if statement like this
		}
	}
